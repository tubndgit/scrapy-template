...
class CustomProxyMiddleware(object):
    def __init__(self, settings):
        if not settings.getbool('PROXY_ENABLED', True):
            raise NotConfigured
        self.proxy = settings.get('PROXY_URL', 'http://127.0.0.1:24000')
...
class ScrapyxProxyMiddleware(object):
    def __init__(self, settings):
        if not settings.getbool('PROXY_ENABLED', True):
            raise NotConfigured
        self.proxies = itertools.cycle(arg_to_iter(settings.get('PROXY_URL', 'http://127.0.0.1:24000')))        
